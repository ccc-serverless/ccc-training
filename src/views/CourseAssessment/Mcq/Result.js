...
export default function Result({ result, isOpen, handleClose, handleGameReset }) {
  ...
  function moveToNextItem() {
    if (params.itemNumber < 3) {
      navigate(
        `/course/learn/${params.activeCourseId}/${params.moduleNumber}/${
          parseInt(params.itemNumber) + 1
        }`
      );
    }
  }
  ...
  return (
    <Modal title="Result" isOpen={isOpen} onClose={handleClose} displayCross={true}>
      ...
      {result.accuracy > result.threshold && params.itemNumber < 3 ? (
        <>
          <div className={style.resultText}>
            You can now proceed to the next level
            <button onClick={moveToNextItem}>Next</button>
          </div>
        </>
      ) : result.accuracy > result.threshold && params.itemNumber == 3 ? (
        <div className={style.resultText}>
          Congratulation! You have completed all the assessments
        </div>
      ) : (
        <div className={style.resultText}>
          You need {result.threshold}% to move to next level
