...
export default function useGameActions({ state, dispatch }) {
  function handleClickCircle(circle) {
    ...
    const { currTurn, numberPool } = uRunData;
    ...
    if (numberPool[poolIndex]) {
      let randIndex = getRandomNumber(0, numberPool[poolIndex].length);
      uRunData.circleData[index].value = numberPool[poolIndex][randIndex];
      uRunData.numberPool[poolIndex].splice(randIndex, 1);
    } else {
      style.visibility = "hidden";
    }

    /* Update the next current answer*/
    uRunData.currAns = uRunData.allNumbers[uRunData.currTurn];
